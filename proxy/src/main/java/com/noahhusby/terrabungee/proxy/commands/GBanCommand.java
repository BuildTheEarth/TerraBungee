    ...
    public void execute(CommandSender sender, String[] args) {
        ...
        playerFuture.thenAccept(tbPlayer -> {
           if(tbPlayer == null) {
            ...
            CompletableFuture<Response> banFuture = TerraBungeeProxy.getInstance().getTerraBungee().getNetworkManager().send(new S2CBanPlayerPacket((sender instanceof ProxiedPlayer) ? ((ProxiedPlayer) sender).getUniqueId() : UUID.fromString("00000000-0000-0000-0000-000000000000"), tbPlayer.getUniqueID(), lengthString, reason.toString()));
            ...
            banFuture.thenAccept(response -> {
               if(response.getCode() == Response.ResponseCode.ERROR) {
                   ...
                   sender.sendMessage(ChatUtil.titleAndCombine(ChatColor.GRAY, "Successfully banned ", ChatColor.YELLOW, tbPlayer.getName(), lengthString.equals("0") ? " permanently" : " for " + lengthString + " days", ChatColor.GRAY, " for ", ChatColor.BLUE, finalReason));
               } else {
