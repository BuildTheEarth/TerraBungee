...
public class GMuteCommand extends Command {
    ...
    public void execute(Invocation invocation) {
        ...
        if (args.length < 3) {
            sender.sendMessage(ChatUtil.USAGE("/gmute <player> <length> <reason>"));
            return;
        }
        ...
        try {
            ...
            sender.sendMessage(ChatUtil.titleAndCombine(NamedTextColor.BLUE, lengthString, NamedTextColor.GRAY, " is not a valid length! Enter the amount of time for the mute (Ex: 1d12h), or enter ", NamedTextColor.YELLOW, "0 ", NamedTextColor.GRAY, "for a permanent mute."));
            return;
        }
        ...
        playerFuture.thenAccept(tbPlayer -> {
            ...
            CompletableFuture<Response> banFuture = TerraBungeeProxy.getInstance().getTerraBungee().getNetworkManager().send(new S2CMutePlayerPacket((sender instanceof Player) ? ((Player) sender).getUniqueId() : UUID.fromString("00000000-0000-0000-0000-000000000000"), tbPlayer.getUniqueID(), length, reason.toString()));
            String finalReason = reason.toString();
            banFuture.thenAccept(response -> {
                if (response.getCode() == Response.ResponseCode.ERROR) {
                    sender.sendMessage(ChatUtil.titleAndCombine(NamedTextColor.YELLOW, tbPlayer.getName(), NamedTextColor.GRAY, " was already muted!"));
                } else if (response.getCode() == Response.ResponseCode.SUCCESS) {
                    sender.sendMessage(ChatUtil.titleAndCombine(NamedTextColor.GRAY, "Successfully muted ", NamedTextColor.YELLOW, tbPlayer.getName(), lengthString.equals("0") ? " permanently" : " for " + DateUtil.getExpandedTimeMessage(length), NamedTextColor.GRAY, " for ", NamedTextColor.BLUE, finalReason));
                } else {
